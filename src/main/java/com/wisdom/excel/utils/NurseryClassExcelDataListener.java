 ...
 * 幼小班
 */
public class NurseryClassExcelDataListener extends AnalysisEventListener<NurseryClassExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(NurseryClassExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<NurseryClassExcelEntity> list = new ArrayList<>();
...
//    private List<AoShuExcelEntity> exceptionList = new ArrayList<>(); // 异常数据
    ...
    public NurseryClassExcelDataListener(StudentService studentService,NurseryClassService nurseryClassService) {
        this.studentService = studentService;
        this.nurseryClassService = nurseryClassService;
    }

    @Override
    public void invoke(NurseryClassExcelEntity data, AnalysisContext context) {
    ...
    public void saveData() {
        NurseryClassExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//        CostDetailsEntity costDetailsEntity = null;
        ...
        if (null != list && list.size() > 0) {
            ...
            for (NurseryClassExcelEntity e : list) {
                ...
                if (e.getBaYue()!=null || "".equals(e.getBaYue())) {
                    baYue = 0;
                }else{
                    baYue = Integer.valueOf(e.getBaYue());
                }
...
//                    monthExpendEntity = new MonthExpendEntity();
...
//                    nurseryClassEntity.setNcId(0);
                    ...
                    if (e.getPrice()!=null && !"".equals(e.getPrice())) {
                        nurseryClassEntity.setPrice(Double.valueOf(e.getPrice()));
                    }
                    //  8月
                    nurseryClassEntity.setAugust(baYue);
                    //  9月
                    nurseryClassEntity.setSeptember(jiuYue);
                    //  10月
                    nurseryClassEntity.setOctober(shiYue);
                    //  11月
                    nurseryClassEntity.setNovember(shiYiYue);
                    //  12月
                    nurseryClassEntity.setDecember(shiErYue);
                    //  1月
                    nurseryClassEntity.setJanuary(yiYue);
                    //  2月
                    nurseryClassEntity.setFebruary(erYue);
                    //  3月
                    nurseryClassEntity.setMarch(sanYue);
                    //  4月
                    nurseryClassEntity.setApril(siYue);
                    //  5月
                    nurseryClassEntity.setMay(wuYue);
                    //  6月
                    nurseryClassEntity.setJune(liuYue);
                    //  课外班
        ...
        NurseryClassExcelDataListener.LOGGER.info("存储数据库成功！");
    }
