 ...
 * 书法
 */
public class CalligraphyExcelDataListener extends AnalysisEventListener<CalligraphyExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CalligraphyExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<CalligraphyExcelEntity> list = new ArrayList<>();

    private int successCount = 0; // 成功量

    private int exceptionCount = 0; // 异常量

    private List<CalligraphyExcelEntity> exceptionList = new ArrayList<>(); // 异常数据

    private StudentService studentService;
    private ClassesService classesService;
    private CostDetailsService costDetailsService;

    public CalligraphyExcelDataListener(StudentService studentService, ClassesService classesService, CostDetailsService costDetailsService) {
...
//    身份证号码正则表达式
    ...
    @Override
    public void invoke(CalligraphyExcelEntity data, AnalysisContext context) {
//        LOGGER.info("解析到一条数据:{}", JSON.toJSONString(data));
    ...
    public void saveData() {
        CalligraphyExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//        ClassesEntity classesEntity = null;
        ...
        if (null != list && list.size() > 0) {
            ...
            for (CalligraphyExcelEntity e : list) {
...
//                    classesEntity = new ClassesEntity();
        ...
        CalligraphyExcelDataListener.LOGGER.info("存储数据库成功！");
    }
     ...
     * @return
     */
    public List<CalligraphyExcelEntity> getExceptionList() {
