 ...
 * 民族舞
 */
public class FolkDanceExcelDataListener extends AnalysisEventListener<FolkDanceExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(FolkDanceExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<FolkDanceExcelEntity> list = new ArrayList<>();
...
//    private List<AoShuExcelEntity> exceptionList = new ArrayList<>(); // 异常数据
    ...
    public FolkDanceExcelDataListener(StudentService studentService, ClassesService classesService,CostDetailsService costDetailsService) {
    ...
    @Override
    public void invoke(FolkDanceExcelEntity data, AnalysisContext context) {
//        LOGGER.info("解析到一条数据:{}", JSON.toJSONString(data));
    ...
    public void saveData() {
        FolkDanceExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//        ClassesEntity classesEntity = null;
        ...
        if (null != list && list.size() > 0) {
            ...
            for (FolkDanceExcelEntity e : list) {
                if ("民族舞-陈璐".equals(e.getCname().trim())) {
                    courseId = 17;
                }else{
                    courseId = 19;
                }
...
//                System.out.println("schoolName======"+e.getSchooltime());
                if (e.getSchooltime() != null && !"".equals(e.getSchooltime())) {
...
//                    classesEntity = new ClassesEntity();
                    ...
                    if (e.getPeriodOfValidity()!=null && !"".equals(e.getPeriodOfValidity())) {
                        try {
                            costDetailsEntity.setPeriodOfValidity(new SimpleDateFormat("yyyy.MM.dd").parse(e.getPeriodOfValidity()));
        ...
        FolkDanceExcelDataListener.LOGGER.info("存储数据库成功！");
    }
