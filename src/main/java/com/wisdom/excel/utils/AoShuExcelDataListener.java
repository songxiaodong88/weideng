 ...
 * 奥数
 */
public class AoShuExcelDataListener extends AnalysisEventListener<AoShuExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AoShuExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<AoShuExcelEntity> list = new ArrayList<>();
    ...
    public AoShuExcelDataListener(StudentService studentService, ClassesService classesService,CostDetailsService costDetailsService) {
    ...
    @Override
    public void invoke(AoShuExcelEntity data, AnalysisContext context) {
//        LOGGER.info("解析到一条数据:{}", JSON.toJSONString(data));
    ...
    public void saveData() {
        AoShuExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//                    classesEntity = new ClassesEntity();
        ...
        AoShuExcelDataListener.LOGGER.info("存储数据库成功！");
    }
