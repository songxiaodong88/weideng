 ...
 * 英语信息
 */
public class EnglishExcelDataListener extends AnalysisEventListener<EnglishExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EnglishExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<EnglishExcelEntity> list = new ArrayList<>();

    private int successCount = 0; // 成功量

    private int exceptionCount = 0; // 异常量

    private List<EnglishExcelEntity> exceptionList = new ArrayList<>(); // 异常数据

    private StudentService studentService;
    private ClassesService classesService;
    private CostDetailsService costDetailsService;

    public EnglishExcelDataListener(StudentService studentService,ClassesService classesService,CostDetailsService costDetailsService) {
...
//    身份证号码正则表达式
    ...
    @Override
    public void invoke(EnglishExcelEntity data, AnalysisContext context) {
//        LOGGER.info("解析到一条数据:{}", JSON.toJSONString(data));
    ...
    public void saveData() {
        EnglishExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//        ClassesEntity classesEntity = null;
        ...
        if (null != list && list.size() > 0) {
            ...
            for (EnglishExcelEntity e : list) {
                if (e.getCname().trim().startsWith("幼儿")) {
                    courseId = 4;
                } else {
                    courseId = 5;
                }
                ...
                sum = jiuYue + (int)(Double.valueOf(shiYue) + Double.valueOf(shiYiYue)) + shiErYue;
                classesId = classesService.selectClassIDByClassName(e.getCname().trim(), e.getSchooltime().trim());
...
//                    classesEntity = new ClassesEntity();
                    ...
                    if (e.getShuFei() != null) {
                        costDetailsEntity.setBookPrice(Double.parseDouble(e.getShuFei()));
                    }else{
                        costDetailsEntity.setBookPrice(0.00);
                    }
    ...
    EnglishExcelDataListener.LOGGER.info("存储数据库成功！");
}
     ...
     * @return
     */
    public List<EnglishExcelEntity> getExceptionList() {
