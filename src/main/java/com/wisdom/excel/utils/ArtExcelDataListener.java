 ...
 * 美术
 */
public class ArtExcelDataListener extends AnalysisEventListener<ArtExcelEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ArtExcelDataListener.class);
    /** 每隔5条存储数据库，实际使用中可以3000条，然后清理list ，方便内存回收 */
    private static final int BATCH_COUNT = 50;

    List<ArtExcelEntity> list = new ArrayList<>();
...
//    private List<AoShuExcelEntity> exceptionList = new ArrayList<>(); // 异常数据
    ...
    public ArtExcelDataListener(StudentService studentService, ClassesService classesService,CostDetailsService costDetailsService) {
    ...
    @Override
    public void invoke(ArtExcelEntity data, AnalysisContext context) {
//        LOGGER.info("解析到一条数据:{}", JSON.toJSONString(data));
    ...
    public void saveData() {
        ArtExcelDataListener.LOGGER.info("{}条数据，开始存储数据库！", list.size());
...
//        ClassesEntity classesEntity = null;
        ...
        if (null != list && list.size() > 0) {
            ...
            for (ArtExcelEntity e : list) {
                if (e.getCname().startsWith("幼儿")) {
                    courseId = 12;
                }else{
                    courseId = 13;
                }
...
//                    classesEntity = new ClassesEntity();
                    ...
                    if (e.getRemark()!=null && !"".equals(e.getRemark())) {
                        costDetailsEntity.setRemark(e.getRemark());
                    }
        ...
        ArtExcelDataListener.LOGGER.info("存储数据库成功！");
    }
